...
class _Loader(object):
    ...
    def dll(self):
        if not self._dll:
            try:
                ...
                # print("jbjct dll is loaded")
            except Exception as e:
                print("failed to load jbjct", e)
        return self._dll
    ...
    def openAllUserDic(self, paths):
        ...
        for i in range(min(len(paths), MAX_USERDIC_COUNT)):
            path = paths[i]
...
class Loader(object):
    ...
    def setUserDic(self, paths):
        ...
        """
        try:
            return self.__d.openAllUserDic(paths)
        except (WindowsError, AttributeError):
            print("failed to load jbjct dll, raise runtime error")
            # raise RuntimeError("failed to access jbjct dll")
            return False

    def clearUserDic(self):
        try:
            self.__d.closeAllUserDic()
        except (WindowsError, AttributeError):
            print("failed to load jbjct dll, raise runtime error")
            # raise RuntimeError("failed to access jbjct dll")
    ...
    def translate(self, text, simplified=True):
        ...
        """
        try:
            return self.__d.transferUnicode(text,
                                            to=CODEPAGE_GB if simplified else CODEPAGE_BIG5)
        except WindowsError or AttributeError as e:
            print("failed to load jbjct dll, raise runtime error", e)
            raise RuntimeError("failed to access jbjct dll")


if __name__ == '__main__':  # DEBUG
    import os
    os.environ['PATH'] += os.pathsep + r"D:\Game\JBeijing7"
    ...
    l.setUserDic((
        u"VnrDictionaries/@jichi/JcUserdic/Jcuser",
        u"VnrDictionaries/@djz020815/JcUserdic/Jcuser",
        u"VnrDictionaries/@najizhimo/JcUserdic/Jcuser",
    ))
    ret = l.translate("魑魅魍魎")
    print(ret)

    ret = l.translate("メルキア")
    print(ret)

    ret = l.translate("せんせい")
    print(ret)
